	...
	function understrap_pagination( $args = array(), $class = 'pagination' ) {
		...
		<nav aria-labelledby="posts-nav-label">
			...
			<ul class="<?php echo esc_attr( $class ); ?>">
				...
				<?php
                $pag_total = count($links);
				foreach ( $links as $key => $link ) {
					?>
                    <?php if (strpos( $link, 'current' ) && $key == 0 ) : ?>
                        <li class="page-item disabled">
                            <a class="prev  page-link" href="#"><?= $args['prev_text'] ?></a>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php if (strpos( $link, 'current' ) && $key == ($pag_total - 1) ) : ?>
                        <li class="page-item disabled">
                            <a class="next page-link" href="#"><?= $args['next_text'] ?></a>
                        </li>
                    <?php endif; ?>
					<?php
				}
